...
class PengadaanController
{
    public function index(): void
    {
        ...
        if (in_array($status, ['Pending', 'Diterima', 'Dibatalkan'])) {
            $sql     .= " AND pg.status = ?";
            $params[] = $status;   // FIX: was $params = $status (overwrote array)
            $types   .= 's';
        }
    ...
    public function create(): void
    {
        ...
        $products = $db->query(
            "SELECT id, kode_barang, nama_barang, harga_beli, satuan FROM product ORDER BY nama_barang"
        )->fetch_all(MYSQLI_ASSOC);
        $error = '';
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!$distributor_id || empty($items)) {
                ...
                try {
                    ...
                    $stmt->bind_param(
                        'siisdss s',  // NOTE: space intentional bug in original — remove it
                        $no_pengadaan, $distributor_id, $user_id, $now,
                        $total_harga, $keterangan, $now, $now
                    );
                    // FIX: correct bind string (no space)
                    $stmt = $db->prepare(
                        "INSERT INTO pengadaan
                            (no_pengadaan, distributor_id, user_id, tanggal_pengadaan,
                             total_harga, status, keterangan, createdAt, updatedAt)
